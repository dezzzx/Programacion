...
public class uso_empleado_con_auemento_sueldo {
	public static void main(String[] args) {
		...
		while (!salir) {
			...
			case 1:
				...
				String[] opcionesEliminar = { "Eliminar Empleado","Aumentar sueldo", "Cancelar" };
				...
				if (opcionEliminar == 0) {
					String indiceEliminarInput = JOptionPane
							.showInputDialog("Ingrese el índice del Empleado a eliminar (1-" + (10 - contador) + "):");
					...
					try {
						...
						if (indiceaumento > 0 && indiceaumento <= (10 - contador)) {
							...
							JOptionPane.showMessageDialog(null, "Sueldo Empleado aumentado");
						} else {
							JOptionPane.showMessageDialog(null, "Índice inválido.");
						}
					} catch (NumberFormatException e) {
...
class Empleado {
	...
	public static int aumentarsueldoEmpleado(Empleado[] empleados, int indice, int contador) {
	    ...
	    double sueldo = empleados[contador-1].dame_sueldo();
	    int nsueldo = (int) (sueldo * (1 + porcentaje));
	    double nuevoSueldo = nsueldo;
	    int numero=contador-1;
	    empleados[contador-1].dime_sueldo(nuevoSueldo,numero);

	    return contador;
	}

	private void dime_sueldo(double nuevoSueldo,int numero) {
		this.sueldo[numero]=nuevoSueldo;
